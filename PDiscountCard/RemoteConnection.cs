...
namespace PDiscountCard
{
    ...
    public class RemoteConnection
    {
        ...
        private void ReceiveCallback(IAsyncResult result)
        {
            ...
            if (bytesRead > 0)
            {
                OnDataRecive((object )getObjectWithByteArray(connection.Buffer));
            }
        ...
        public bool SendData(string ServerName, int port, object  Fi, bool TryConnect)
        {
            try
            {
             //   IPHostEntry RemotelMachineInfo = Dns.GetHostEntry(ServerName);
                IPEndPoint RemoteEndpoint = new IPEndPoint(IPAddress.Loopback   , port);
                //IPEndPoint RemoteEndpoint = new IPEndPoint(IPAddress.Loopback   RemotelMachineInfo.AddressList[0], port);
                //Utils .ToCardLog ()
